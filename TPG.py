...
# #######                                                 #
#                                                         #
# v 0.1.7 - ??/??/???? en cours de développement          #
#         - Some code rewriting                           #
#                                                         #
# v 0.1.6 - 13/03/2002                                    #
#         - Bug fixes                                     #
...
class TPParser(ToyParser, ):
	...
	def __call__(self, input, *args):
		...
		self.setInput(input)
		return self.START(self.tpg_Pos(),*args)[1]
	...
	def CODE(self, tpg_p0, ):
		"""CODE -> '{{((?:}?[^}]+)*)}}' ."""
		tpg_p1, (c,) = self.tpg_eat(tpg_p0, r'{{((?:}?[^}]+)*)}}', 2)
...
class TagParser(ToyParser, ):
	...
	def START(self, tpg_p0, ):
		...
		while 1:
			try:
				try:
					tpg_p4, (name,) = self.tpg_lex_eat(tpg_p0, r'\s*#\s*<([^>\n]*)>.*\n', 2)
					...
					while 1:
						try:
							...
							tpg_p5 = tpg_p4
							break
					tpg_p2, (endname,) = self.tpg_lex_eat(tpg_p5, r'\s*#\s*<\s*/([^>\n]*)>.*\n', 2)
