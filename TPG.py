...
class ArgParser(ToyParser, ):
	...
	def START(self, tpg_p0, ):
		"""START -> ( '([^\s]|\\\s)+\.g' | '-o' '([^\s]|\\\s)+\.py' | '-(v+)' )* '$' ."""
		input_file, output_file, verbosity = None, None, 0 
		while 1:
			try:
				try:
					try:
						tpg_p2, st = self.tpg_eat(tpg_p0, r'([^\s]|\\\s)+\.g', 1)
					...
					tpg_p2, (v,) = self.tpg_eat(tpg_p0, r'-(v+)', 2)
					
					self.check(verbosity is 0)
					verbosity = len(v)
		...
		if output_file is None: output_file = input_file[:-2]+'.py'
		
		tpg_p1,  = self.tpg_eat(tpg_p2, r'$', 0)
		return tpg_p1, (input_file, output_file, verbosity,)
...
if __name__ == "__main__":
	...
	try:
		argParser = ArgParser()
		(grammar_file, output_file, _v) = argParser(' '.join(sys.argv[1:]))
